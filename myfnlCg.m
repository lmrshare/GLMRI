function x = myfnlCg(x0,problem, params)
%-----------------------------------------------------------------------
%
...
% backtracking line-search
	...
	[FTXFMtx, FTXFMtdx, DXFMtx, DXFMtdx,FIXDLtx,FIXDLtdx] = preobjective(x, dx, problem);
	f0 = objective(FTXFMtx, FTXFMtdx,FIXDLtx,FIXDLtdx,DXFMtx, DXFMtdx,x,dx, 0, problem, params);
	t = t0;
        [f1, ERRobj, RMSerr]  =  objective(FTXFMtx, FTXFMtdx,FIXDLtx,FIXDLtdx,DXFMtx, DXFMtdx,x,dx, t, problem, params);
...
function [FTXFMtx, FTXFMtdx, DXFMtx, DXFMtdx,FIXDLtx,FIXDLtdx] = preobjective(x, dx, problem)
...
FIXDLtx = problem.A.psi'*x;                         %modify by @LIN
FIXDLtx = FIXDLtx(:);
FIXDLtdx = problem.A.psi'*dx;
FIXDLtdx = FIXDLtdx(:);
...
function [res, obj, RMS] = objective(FTXFMtx, FTXFMtdx,FIXDLtx,FIXDLtdx ,DXFMtx, DXFMtdx, x,dx,t, problem, params)
...
DL = FIXDLtx(:) + t*FIXDLtdx(:) - problem.zdl;              %modify by @LIN
DL = DL'*DL;

if problem.DLWeight
    DL = problem.DLWeight * problem.DLbeta * DL;
else
    DL = 0;
end
...
gradDL = gDL(x,problem);                            %modify by @LIN
if problem.xfmWeight
...
grad = (gradObj + gradDL + problem.xfmWeight.*gradXFM + problem.TVWeight.*gradTV);


function grad = gDL(x,problem)                      %add by @LIN

grad = problem.A.psi'*x;
grad = problem.A.psi*(grad(:) - problem.zdl);
grad = 2 * problem.DLWeight * problem.DLbeta * grad(:);
